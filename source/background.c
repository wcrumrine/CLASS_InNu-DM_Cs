...
int background_functions(
  ...
  if (pba->has_nudm == _TRUE_) {
    pvecback[pba->index_bg_rho_nudm] = pba->Omega0_nudm * pow(pba->H0,2) / pow(a_rel, 3);
  ...
  if (pba->has_ncdm == _TRUE_) {
    ...
    for(n_ncdm=0; n_ncdm<pba->N_ncdm; n_ncdm++){
      ...
      if (pba->has_nudm == _TRUE_) {
        pvecback[pba->index_bg_A_nudm1+n_ncdm] = a*pba->u_ncdmdm[n_ncdm]*_sigma_*_Mpc_over_m_*9.47e5* pvecback[pba->index_bg_rho_nudm];
...
int background_w_fld(
  ...
  case EDE:
    ...
    if (pba->has_nudm == _TRUE_) Omega_m += pba->Omega0_nudm;
    if (pba->has_dcdm == _TRUE_)
...
int background_output_data(
  ...
  for (index_tau=0; index_tau<pba->bt_size; index_tau++){
    ...
    class_store_double(dataptr,pvecback[pba->index_bg_rho_nudm],pba->has_nudm,storeidx);
    if (pba->has_ncdm == _TRUE_){
      for (n=0; n<pba->N_ncdm; n++){
